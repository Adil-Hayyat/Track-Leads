...
function Sidebar({ page, user, navigate, onAdd, onSignOut, collapsed, onToggle }) {
  return <aside className={`sidebar ${collapsed ? 'sidebar-collapsed' : ''}`}><div className="sidebar-brand"><Logo compact={collapsed} /></div><nav className="side-nav" aria-label="Primary navigation"><button className={page === 'dashboard' ? 'active' : ''} aria-label="Overview" title="Overview" onClick={() => navigate('dashboard')}><span className="sidebar-label">Overview</span></button><button className={page === 'leads' ? 'active' : ''} aria-label="Leads" title="Leads" onClick={() => navigate('leads')}><span className="sidebar-label">Leads</span></button><button className={page === 'add-lead' ? 'active' : ''} aria-label="Add New Lead" title="Add New Lead" onClick={onAdd}><span className="sidebar-label">Add New Lead</span></button></nav><div className="sidebar-spacer" /><button className="sidebar-logout" aria-label="Logout" title="Logout" onClick={onSignOut}><span>↪</span><span className="sidebar-label">Logout</span></button><div className="profile"><span className="profile-avatar">{getInitials(user)}</span><span className="sidebar-label"><b>{getUserLabel(user)}</b><small>{user?.email || 'Authenticated user'}</small></span></div></aside>
}

...
function AuthPage({ mode, navigate, onAuthenticated }) {
  ...
  async function submit(event) {
    ...
    const normalizedEmail = email.trim().toLowerCase()
    if (!normalizedEmail) { setError('Work email is required.'); return }
...
function Sidebar({ page, user, navigate, onAdd, onSignOut, collapsed, onToggle }) {
  return <aside className={`sidebar ${collapsed ? 'sidebar-collapsed' : ''}`}><div className="sidebar-brand"><Logo compact={collapsed} /><button className="collapse-btn" aria-label={collapsed ? 'Expand sidebar' : 'Collapse sidebar'} onClick={onToggle}>{collapsed ? '>' : '<'}</button></div><div className="workspace-switch"><span className="workspace-icon">{getInitials(user).slice(0, 1)}</span><span className="sidebar-label"><small>Workspace</small><b>{getUserLabel(user)}'s workspace</b></span><span className="sidebar-label">v</span></div><nav className="side-nav" aria-label="Primary navigation"><p className="sidebar-label">Workspace</p><button className={page === 'dashboard' ? 'active' : ''} aria-label="Overview" title="Overview" onClick={() => navigate('dashboard')}><span>[]</span><span className="sidebar-label">Overview</span></button><button className={page === 'leads' ? 'active' : ''} aria-label="Leads" title="Leads" onClick={() => navigate('leads')}><span>+</span><span className="sidebar-label">Leads</span></button><button className={page === 'add-lead' ? 'active' : ''} aria-label="Add New Lead" title="Add New Lead" onClick={onAdd}><span>+</span><span className="sidebar-label">Add New Lead</span></button></nav><div className="sidebar-spacer" /><button className="sidebar-logout" aria-label="Logout" title="Logout" onClick={onSignOut}><span>↪</span><span className="sidebar-label">Logout</span></button><div className="profile"><span className="profile-avatar">{getInitials(user)}</span><span className="sidebar-label"><b>{getUserLabel(user)}</b><small>{user?.email || 'Authenticated user'}</small></span></div></aside>
}
...
function App() {
  ...
  function openCreate() { setEditingLead(null); setForm(emptyForm); setError(''); setView('add-lead'); setFormOpen(true) }
  ...
  async function saveLead(event) {
    ...
    if (userError || !userData.user) {
      logSupabaseError('verify lead session', userError)
      setError('Your session has expired. Please sign in again before saving a lead.')
      setSaving(false)
      return
    }
    ...
    try {
      ...
      if (response.error) { logSupabaseError(editingLead ? 'update lead' : 'create lead', response.error); setError(response.error.code === '42501' ? 'Supabase rejected this lead because the live INSERT policy is not allowing your authenticated role. Run the authenticated INSERT migration in Supabase SQL Editor.' : response.error.code === '23514' ? 'The selected owner or status is not valid.' : 'We could not save this lead. Check the table permissions and try again.') }
  ...
  return <><Dashboard page={view} user={session.user} navigate={setView} leads={leads} filteredLeads={filteredLeads} isLoading={isLoading} error={error} search={search} setSearch={setSearch} statusFilter={statusFilter} setStatusFilter={setStatusFilter} onAdd={openCreate} onEdit={openEdit} onDelete={deleteLead} deletingId={deletingId} onSignOut={signOut} />{formOpen && <LeadModal editingLead={editingLead} form={form} setForm={setForm} error={error} saving={saving} onClose={() => { if (!saving) { setFormOpen(false); setView('dashboard') } }} onSave={saveLead} />}</>
}
